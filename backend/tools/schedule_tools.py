...
_deadline_index = None
_event_index = None
_term_index = None

def _get_deadline_index():
    """Get or initialize the deadline embedding index"""
    global _deadline_index
    
    if _deadline_index is None:
        _deadline_index = EmbeddingIndex("academic_deadlines", cache_dir="/tmp")
        
        # Create items for embedding
        deadline_items = []
        for name, deadline in ACADEMIC_DEADLINES.items():
            # Create a rich text representation
            date_str = deadline["date"].strftime("%B %d, %Y")
            text = f"{name}: {deadline['description']}. Date: {date_str}"
            
            if "instructions" in deadline:
                text += f". {deadline['instructions']}"
        ...
        # Create embeddings
        _deadline_index.load_or_create(deadline_items, text_key='text', id_key='id', force_rebuild=True)
    
    return _deadline_index

def _get_event_index():
    """Get or initialize the event embedding index"""
    global _event_index
    
    if _event_index is None:
        _event_index = EmbeddingIndex("university_events", cache_dir="/tmp")
        
        # Create items for embedding
        event_items = []
        for name, event in UNIVERSITY_EVENTS.items():
            # Create a rich text representation
            start_date = event["start_date"].strftime("%B %d, %Y")
            text = f"{name}: {event['description']}. Date: {start_date}. Location: {event['location']}"
        ...
        # Create embeddings
        _event_index.load_or_create(event_items, text_key='text', id_key='id', force_rebuild=True)
    
    return _event_index

def _get_term_index():
    """Get or initialize the term embedding index"""
    global _term_index
    
    if _term_index is None:
        _term_index = EmbeddingIndex("academic_calendar", cache_dir="/tmp")
        
        # Create items for embedding
        term_items = []
        for name, term in ACADEMIC_CALENDAR.items():
            # Create a rich text representation
            start_date = term["start_date"].strftime("%B %d, %Y")
            end_date = term["end_date"].strftime("%B %d, %Y")
            text = f"{name}: Start: {start_date}, End: {end_date}. {term.get('description', '')}"
        ...
        # Create embeddings
        _term_index.load_or_create(term_items, text_key='text', id_key='id', force_rebuild=True)
    
    return _term_index

def get_deadline_info(deadline_name: str) -> str:
    """
    Get information about a specific academic deadline using semantic search.
    
    Args:
        deadline_name: The deadline to look up (can be natural language)
        
    Returns:
        String with deadline information
    """
    # Get the deadline index
    deadline_index = _get_deadline_index()
    
    # Search for the deadline
    results = deadline_index.search(deadline_name, top_k=1, threshold=0.4)
    
    if not results:
        return f"I don't have information about '{deadline_name}'. Would you like to know about our registration deadlines or final exam schedules?"
    ...
    result = f"Information about {name} (match: {score:.2f}):\n"
    result += f"Date: {date_str}\n"
    result += f"Description: {deadline['description']}\n"
    
    if "instructions" in deadline:
        result += f"Instructions: {deadline['instructions']}\n"
        
    return result

def get_event_info(event_name: str) -> str:
    """
    Get information about a specific university event using semantic search.
    
    Args:
        event_name: The event to look up (can be natural language)
        
    Returns:
        String with event information
    """
    # Get the event index
    event_index = _get_event_index()
    
    # Search for the event
    results = event_index.search(event_name, top_k=1, threshold=0.4)
    
    if not results:
        return f"I don't have information about '{event_name}'. Would you like to know about our orientation week or graduation ceremony?"
